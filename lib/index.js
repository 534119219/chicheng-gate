...
function apply(ctx) {
  ...
  const extra = readEarlySetting("extraHosts", "");
  const host = enabled ? "0.0.0.0" : "127.0.0.1";
  const trustedHosts = enabled ? Array.from(new Set([...(typeof dhcp === "string" && dhcp !== "" ? [dhcp] : []), ...lanAddresses(), ...parseHosts(extra)])) : [];
  ...
  let current = { consented: false, remoteEnabled: false, dhcpAddress: "", extraHosts: "", mobileUi: true };
