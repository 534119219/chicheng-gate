...
import { readFileSync, writeFileSync, existsSync, mkdirSync, copyFileSync, readdirSync, statSync, openSync, unlinkSync } from "node:fs";
...
const name = "chicheng-gate";
const NS = "chicheng-gate";
const inject = [];
...
const SettingsSchema = z.object({
  ...
  frpcTrustedHosts: z.string().default(""),
...
function readEarlySetting(field, fallback) {
  ...
  for (const line of ls) {
    ...
    if (inNs) {
      ...
      if (m) {
        let v = m[1].trim();
        // Strip an inline comment: a `#` that starts a comment (preceded by
        // whitespace or at the value start) — but not one inside quotes.
        let quote = null;
        let commentIdx = -1;
        for (let i = 0; i < v.length; i++) {
          ...
          if (ch === "#" && (i === 0 || /\s/.test(v[i - 1]))) { commentIdx = i; break; }
        }
        ...
        if ((v.startsWith('"') && v.endsWith('"')) || (v.startsWith("'") && v.endsWith("'"))) {
          // Unescape single/double-quoted YAML scalars (best effort).
...
class SessionStore {
  constructor(ttlSec = 7 * 24 * 3600, file = join(dshHome(), "chicheng-gate-sessions.json")) {
    this.ttlSec = ttlSec;
    this.file = file;
  ...
  _load() {
    try {
      ...
      for (const [id, row] of Object.entries(raw || {})) {
        if (row && typeof row.exp === "number" && row.exp > now) this.sessions.set(id, row);
      }
    } catch {}
  }
  _persist() {
    try {
      const obj = {};
      for (const [id, row] of this.sessions) obj[id] = row;
  ...
  get(token) {
    ...
    if (row.exp <= Date.now()) { this.sessions.delete(id); this._persist(); return undefined; }
    return row;
  }
  drop(token) {
    if (typeof token === "string") { this.sessions.delete(hashToken(token)); this._persist(); }
  }
  dropAll() { this.sessions.clear(); this._persist(); }
}
...
const LOGIN_PATH = "/chicheng-gate/login";
const LOGOUT_PATH = "/chicheng-gate/logout";
const PASSWORD_PATH = "/chicheng-gate/password";
const STATUS_PATH = "/chicheng-gate/status";
...
// frpc integration.
// ---------------------------------------------------------------------------
const FRPC_UA = { "User-Agent": "chicheng-gate" };
...
async function downloadFrpc() {
  ...
  try {
    ...
    if (chkRes.ok) {
      ...
      console.warn(`[chicheng-gate] frp checksums fetch failed (HTTP ${chkRes.status}); skipping verification`);
    }
  } catch (e) {
    if (e.message.startsWith("frp SHA256 校验失败")) throw e;
    throw new Error("SHA256 校验文件获取/解析失败: " + e.message);
  }
...
function killStaleFrpc() {
  ...
  // Remove the pid file instead of leaving a 0-byte stub behind.
...
// Wait for a child process to exit (bounded), so a restart never leaves two
// frpc instances overlapping. Resolves true if it exited, false on timeout.
function waitForChildExit(child, timeoutMs = 3000) {
  if (!child || child.exitCode !== null || child.signalCode !== null) return Promise.resolve(true);
  return new Promise((resolve) => {
    const timer = setTimeout(() => { cleanup(); resolve(false); }, timeoutMs);
    const onExit = () => { cleanup(); resolve(true); };
    const cleanup = () => { clearTimeout(timer); child.removeListener("exit", onExit); child.removeListener("error", onExit); };
...
const ALWAYS_PATCHES = [
  ...
  // chicheng-gate 已在 apply() 中通过 ctx.inject(["settings"]).settings.register(NS, SettingsSchema)
  // 注册命名空间，因此不再需要 apiproxy 补丁（旧补丁会静默失配）。
  {
    file: "node_modules/@deepseek-ai/dsh-client-ui-settings-general/lib/client.js",
    apply: (text) => text.includes('id === "chicheng-gate"') ? text : text.replace(NAV_GATE_FALLBACK, NAV_GATE_ICON + NAV_GATE_FALLBACK)
  }
];
...
function applyPatches(root, patches) {
  for (const patch of patches) {
    ...
    if (!existsSync(file)) { console.warn("[chicheng-gate] patch target missing: " + file); continue; }
    const bak = file + ".chicheng-gate.bak";
...
function restartDshWeb() {
  ...
  // Graceful restart where the platform supports it: on POSIX, SIGTERM lets
  // DSH's own process-shutdown handler run its bounded dispose. On Windows,
  // Node's process.kill(SIGTERM) is a hard kill (no handler fires), so we
  // fall back to taskkill /F — same behavior as before. A detached node
  // script signals the process, waits for it to actually exit, then
  // relaunches `dsh web`.
  ...
  const script = `
    ...
    function relaunch() {
      try {
        const c = spawn("cmd", ["/c", "dsh", "web"], { detached: true, stdio: "ignore", windowsHide: true });
        c.unref();
      } catch (e) {}
    }
    function killIt() {
      if (win) {
        try { spawn("taskkill", ["/F", "/PID", String(pid)], { stdio: "ignore" }); } catch (e) { relaunch(); }
      } else {
        try { process.kill(pid, "SIGTERM"); } catch (e) { relaunch(); }
      }
    }
    function tick() {
      if (gone()) { relaunch(); return; }
      if (!sent) {
        sent = true;
        killIt();
      }
  ...
  `;
  console.error("[chicheng-gate] restart requested, pid=" + pid);
  try {
    const child = spawn(process.execPath, ["-e", script], { detached: true, stdio: "ignore", windowsHide: true });
    child.unref();
  } catch (error) { console.error("[chicheng-gate] restart spawn failed:", error); }
}
...
// For WebSocket upgrades we must KEEP `connection: upgrade` and `upgrade: websocket`
// (stripping them would make the upstream refuse the handshake), but still drop the
// other hop-by-hop headers (content-length, transfer-encoding, keep-alive, ...) that
// could confuse the upstream server.
function stripHopHeadersForUpgrade(headers) {
  const out = {};
  for (const key of Object.keys(headers)) {
    if (HOP_HEADERS.has(key) && key !== "connection" && key !== "upgrade") continue;
    out[key] = headers[key];
  }
  return out;
}
...
function startTunnelGateway(opts) {
  ...
  server.on("request", async (req, res) => {
    try {
      ...
      const upstream = httpRequest({
        ...
        res.writeHead(upRes.statusCode, stripHopHeaders(upRes.headers));
        upRes.pipe(res);
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    const upstream = httpRequest({
      ...
      method: req.method,
      path: req.url,
      headers: stripHopHeadersForUpgrade(req.headers)
    });
  ...
  server.listen(listenPort, "127.0.0.1", () => {
    console.error("[chicheng-gate] tunnel gateway listening on 127.0.0.1:" + listenPort);
  });
...
function apply(ctx) {
  const initial = {
    ...
    frpcTrustedHosts: readEarlySetting("frpcTrustedHosts", "") || "",
    panelPasswordHash: readEarlySetting("panelPasswordHash", "") || ""
  };

  const enabled = initial.remoteEnabled;
  const mobileUi = initial.mobileUi;
  const dhcp = initial.dhcpAddress;
  const host = enabled ? "0.0.0.0" : "127.0.0.1";
  const trustedHosts = enabled ? Array.from(new Set([...(typeof dhcp === "string" && dhcp !== "" ? [dhcp] : []), ...lanAddresses(), "0.0.0.0", ...parseHosts(initial.frpcServerAddr), ...parseHosts(initial.frpcTrustedHosts)])) : [];
  ...
  if (root) {
    ...
    } catch (error) { console.error("[chicheng-gate] source patch error:", error); }
  } else {
    console.error("[chicheng-gate] cannot locate harness checkout; source patches skipped");
  }

  let current = { ...initial };
  let settingsScope = null;

  const sessions = new SessionStore({ ttlSec: 7 * 24 * 3600 });
  ...
  const applyFrpcState = async () => {
    ...
    // Let the previous instance fully exit before starting a new one, so two
    // frpc processes never fight over the same local port.
    if (old) await waitForChildExit(old);
    ...
    try {
      ...
      console.error("[chicheng-gate] frpc start failed:", e.message);
    }
  };
  ...
  ctx.inject(["webServer"], (sctx) => {
    ...
    sctx.effect(() => () => { try { tunnelGateway.close(); } catch {} }, "chicheng-gate: tunnel gateway");

    sctx.effect(() => installServerGate(server, decide), "chicheng-gate: gate");

    sctx.webServer.tapIndex((html) => {
      if (html.includes("data-chicheng-gate")) return html;
