...
function applyPatches(root, patches) {
  for (const patch of patches) {
    ...
    if (!existsSync(file)) { console.warn("[dsh-remote-access] patch target missing: " + file); continue; }
    const bak = file + ".dsh-remote-access.bak";
...
function apply(ctx) {
  ...
  if (root) {
    ...
    } catch (error) { console.error("[dsh-remote-access] source patch error:", error); }
  } else {
    console.error("[dsh-remote-access] cannot locate harness checkout; source patches skipped");
  }
  ...
  ctx.inject(["webServer"], (sctx) => {
    sctx.webServer.tapIndex((html) => {
      if (html.includes("data-dsh-remote-access")) return html;
    ...
    sctx.effect(() => sctx.webServer.register({
      kind: "prefix",
      path: "/remote-access/dhcp",
      handler: async (req, res) => {
        const ips = lanAddresses();
        const body = JSON.stringify({ ok: true, address: ips[0] || "", addresses: ips });
        res.writeHead(200, { "content-type": "application/json; charset=utf-8" });
        res.end(body);
      }
    }), "dsh-remote-access: dhcp route");
  });
}
