...
function restartDshWeb() {
  const script = 'Start-Sleep -Seconds 2; Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like "*bin.js*web*" } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }; Start-Sleep -Seconds 1; Start-Process -FilePath "cmd.exe" -ArgumentList "/c","dsh web" -WindowStyle Hidden';
  try {
    const child = spawn("powershell", ["-NoProfile", "-Command", script], { detached: true, stdio: "ignore", windowsHide: true });
    child.unref();
