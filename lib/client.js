window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      nav: "Lan Gate",
      title: "Lan Gate",
      intro: "让 Web GUI 可通过局域网访问，并优化手机端显示。",
      ...
      extraHostsTitle: "额外可信主机（FRP / 反向代理）",
      extraHostsDesc: "通过 FRP 或反向代理从公网访问时，填入公网域名或 IP（多个用逗号/空格分隔），例如 frp.example.com 或 1.2.3.4:3080。修改后需重启 dsh web 生效。",
      extraHostsPlaceholder: "例如 frp.example.com 或 1.2.3.4:3080"
    };

    var en = {
      nav: "Lan Gate",
      title: "Lan Gate",
      intro: "Expose the Web GUI on the LAN and polish the mobile UI.",
      ...
      extraHostsTitle: "Extra trusted hosts (FRP / reverse proxy)",
      extraHostsDesc: "When accessing from the public internet via FRP or a reverse proxy, enter the public domain or IP (comma/space separated), e.g. frp.example.com or 1.2.3.4:3080. Restart dsh web to apply.",
      extraHostsPlaceholder: "e.g. frp.example.com or 1.2.3.4:3080"
    };
    ...
    function RemoteAccessSection(props) {
      ...
      var extraHosts = typeof value.extraHosts === "string" ? value.extraHosts : "";
      ...
      return createElement("div", { className: "_WvWnq_section" },
        createElement("div", { style: { padding: "2px 0 12px", borderBottom: "1px solid var(--dsw-alias-divider, rgba(0,0,0,0.08))", marginBottom: "10px" } },
        ...
        createElement("div", { style: { display: "flex", alignItems: "center", gap: "6px", padding: "8px 0" } },
          ...
          createElement(Button, { size: "sm", variant: "outline", onClick: restartDsh, style: { borderRadius: "6px" } }, t("restartButton"))
        ),
        createElement(ToggleRow, {
          checked: mobileUi,
        ...
        createElement("div", { style: { padding: "8px 0" } },
          createElement("div", { style: { fontSize: 14, fontWeight: 500, color: "var(--dsw-alias-label-primary)" } }, t("extraHostsTitle")),
          createElement("div", { style: { fontSize: 12, color: "var(--dsw-alias-label-secondary)", lineHeight: 1.5, marginTop: "2px" } }, t("extraHostsDesc")),
          createElement("div", { style: { marginTop: "8px" } },
            createElement(Input, {
              value: extraHosts,
              placeholder: t("extraHostsPlaceholder"),
              onChange: function (e) { set("extraHosts", e.target.value); },
              style: { width: "100%" }
            })
          )
        )
